...
od420_allmn <- tapply(treatfinal$OD420, paste(treatfinal$Water, treatfinal$Plant, treatfinal$Micr),mean,na.rm=T)
od420_allse <- tapply(treatfinal$OD420, paste(treatfinal$Water, treatfinal$Plant, treatfinal$Micr),std.error)
od600_allmn <- tapply(treatfinal$OD600, paste(treatfinal$Water, treatfinal$Plant, treatfinal$Micr),mean,na.rm=T)
od600_allse <- tapply(treatfinal$OD600, paste(treatfinal$Water, treatfinal$Plant, treatfinal$Micr),std.error)
...
par(oma=c(1,0,1,1))
par(mar=c(0,4,0,0))
plot(area_allmn[area_order] ~ c(1:length(area_allmn)), pch =NA, xlab="",ylab="",xaxt="n", ylim=range(c(area_allmn-area_allse,area_allmn+area_allse)) )
...
plot(green_allmn[green_order] ~ c(1:length(green_allmn)), pch =NA, xlab="",ylab="",xaxt="n", ylim=range(c(green_allmn-green_allse,green_allmn+green_allse)) )
...
plot(od420_allmn[od420_order] ~ c(1:length(od420_allmn)), pch =NA, xlab="",ylab="",xaxt="n", ylim=range(c(od420_allmn-od420_allse,od420_allmn+od420_allse)) )
	...
	abline(h=mean(unique(treatfinal$OD420.BlankAverage)),lty=3)
	mtext("OD 420 nm", side=2,line=2)
plot(od600_allmn[od600_order] ~ c(1:length(od600_allmn)), pch =NA, xlab="",ylab="",xaxt="n", ylim=range(c(od600_allmn-od600_allse,od600_allmn+od600_allse)) )
	...
	abline(h=mean(unique(treatfinal$OD600.BlankAverage)),lty=3)
	mtext("OD 600 nm", side=2,line=2)
	text(-1,0.17,"Water source", adj=0)
	legend(-2,0.17,sitenames, adj = c(0,0.5),pch=c(22,23,24,25),bty="n")
	text(24,0.17,"Duckweed source", adj=0)
	legend(22,0.17,sitenames,adj = c(0,0.5), lty=1, col=sourcecols[c(1,4,2,3)],bty="n")
	text(49,0.17,"Microbe source", adj=0)
	legend(47,0.17,c(sitenames,"None"),adj = c(0,0.5),fill=sourcecols_micr[c(1,5,2,4,3)],bty="n")
dev.off()
...
plot(od600_allmn[od600_order] ~ c(1:length(od600_allmn)), pch =NA, xlab="",ylab="",xaxt="n", ylim=range(c(od600_allmn-od600_allse,od600_allmn+od600_allse)) )
	...
	abline(h=mean(unique(treatfinal$OD600.BlankAverage)),lty=3)
	mtext("OD 600 nm", side=2,line=2)
	text(-1,0.17,"Source sites matching", adj=0)
 	legend(-2,0.17,c("All", "Duckweeds and Water", "Microbes and Water","Duckweeds and Microbes", "None"), adj = c(0,0.5),fill=unique(samecols),bty="n")
 	legend(47,0.17,c("Uninoculated","Inoculated"),adj = c(0,0.5),pch=c(1,17),bty="n")
dev.off()
...
od420_dmn <- tapply(treatfinal$OD420, treatfinal$Plant,mean,na.rm=T)
od420_dse <- tapply(treatfinal$OD420, treatfinal$Plant,std.error)
od600_dmn <- tapply(treatfinal$OD600, treatfinal$Plant,mean,na.rm=T)
od600_dse <- tapply(treatfinal$OD600, treatfinal$Plant,std.error)
...
od420_mmn <- tapply(treatfinal$OD420, treatfinal$Microbe,mean,na.rm=T)
od420_mse <- tapply(treatfinal$OD420, treatfinal$Microbe,std.error)
od600_mmn <- tapply(treatfinal$OD600, treatfinal$Microbe,mean,na.rm=T)
od600_mse <- tapply(treatfinal$OD600, treatfinal$Microbe,std.error)
...
od420_wmn <- tapply(treatfinal$OD420, treatfinal$Water,mean,na.rm=T)
od420_wse <- tapply(treatfinal$OD420, treatfinal$Water,std.error)
od600_wmn <- tapply(treatfinal$OD600, treatfinal$Water,mean,na.rm=T)
od600_wse <- tapply(treatfinal$OD600, treatfinal$Water,std.error)
...
par(oma=c(8,4,2,1))
par(mar=c(0,0,0,0))
plot(area_dmn ~ c(1,4,3,2), pch =16, xlab="",ylab="",xaxt="n", ylim=c(9,13.5),col=sourcecols,xlim=c(0.5,4.5),cex=1.5 )
	arrows(c(1,4,3,2),area_dmn-area_dse, y1=area_dmn+area_dse, length=0,col=sourcecols,lwd=2)
	mtext(expression("Frond area, mm"^2), side=2,line=2)
	text(1:4,13.3,areafixpt)
	mtext("Plant Source",side=3,line=0)
plot(area_mmn ~ c(1,4,5,3,2), pch =16, xlab="",ylab="",xaxt="n", yaxt="n", ylim=c(9,13.5),col=sourcecols_micr2 ,xlim=c(0.5,5.5) ,cex=1.5 )
	arrows(c(1,4,5,3,2),area_mmn-area_mse, y1=area_mmn+area_mse, length=0,col=sourcecols_micr2,lwd=2)
	text(1:5,13.3,areafixmt)
	mtext("Microbe Source",side=3,line=0)
plot(area_wmn ~ c(1,4,3,2), pch =16, xlab="",ylab="",xaxt="n", yaxt="n", ylim=c(9,13.5),col=sourcecols ,xlim=c(0.5,4.5) ,cex=1.5 )
	arrows(c(1,4,3,2),area_wmn-area_wse, y1=area_wmn+area_wse, length=0,col=sourcecols,lwd=2)
	text(1:4,13.3,areafixwt)
	mtext("Water Source",side=3,line=0)
plot(green_dmn ~ c(1,4,3,2), pch =16, xlab="",ylab="",xaxt="n", ylim=c(41,46.5),col=sourcecols,xlim=c(0.5,4.5) ,cex=1.5 )
	arrows(c(1,4,3,2),green_dmn-green_dse, y1=green_dmn+green_dse, length=0,col=sourcecols,lwd=2)
	mtext("%Greenness", side=2,line=2)
	text(2.5,46.2,"n.s.")
plot(green_mmn ~ c(1,4,5,3,2), pch =16, xlab="",ylab="",xaxt="n", yaxt="n", ylim=c(41,46.5),col=sourcecols_micr2 ,xlim=c(0.5,5.5) ,cex=1.5 )
	arrows(c(1,4,5,3,2),green_mmn-green_mse, y1=green_mmn+green_mse, length=0,col=sourcecols_micr2,lwd=2)
	text(1:5,46.2,greenfixmt)
plot(green_wmn ~ c(1,4,3,2), pch =16, xlab="",ylab="",xaxt="n", yaxt="n", ylim=c(41,46.5),col=sourcecols ,xlim=c(0.5,4.5) ,cex=1.5 )
	arrows(c(1,4,3,2),green_wmn-green_wse, y1=green_wmn+green_wse, length=0,col=sourcecols,lwd=2)
	text(2.5,46.2,"n.s.")
plot(od420_dmn ~ c(1,4,3,2), pch =16, xlab="",ylab="",xaxt="n", ylim=c(0.06,0.145),col=sourcecols,xlim=c(0.5,4.5) ,cex=1.5 )
	arrows(c(1,4,3,2),od420_dmn-od420_dse, y1=od420_dmn+od420_dse, length=0,col=sourcecols,lwd=2)
	mtext("OD 420 nm", side=2,line=2)
	text(1:4,0.14,od420fixpt)
plot(od420_mmn ~ c(1,4,5,3,2), pch =16, xlab="",ylab="",xaxt="n", yaxt="n", ylim=c(0.06,0.145),col=sourcecols_micr2 ,xlim=c(0.5,5.5) ,cex=1.5 )
	arrows(c(1,4,5,3,2),od420_mmn-od420_mse, y1=od420_mmn+od420_mse, length=0,col=sourcecols_micr2,lwd=2)
	text(1:5,0.14,od420fixmt)
plot(od420_wmn ~ c(1,4,3,2), pch =16, xlab="",ylab="",xaxt="n", yaxt="n", ylim=c(0.06,0.145),col=sourcecols ,xlim=c(0.5,4.5) ,cex=1.5 )
	arrows(c(1,4,3,2),od420_wmn-od420_wse, y1=od420_wmn+od420_wse, length=0,col=sourcecols,lwd=2)
	text(1:4,0.14,od420fixwt)
plot(od600_dmn ~ c(1,4,3,2), pch =16, xlab="",ylab="",xaxt="n", ylim=c(0.04,0.105),col=sourcecols,xlim=c(0.5,4.5) ,cex=1.5 )
	...
	text(1:4,0.10,od600fixpt)
plot(od600_mmn ~ c(1,4,5,3,2), pch =16, xlab="",ylab="",xaxt="n", yaxt="n", ylim=c(0.04,0.105),col=sourcecols_micr2 ,xlim=c(0.5,5.5) ,cex=1.5 )
	...
	text(1:5,0.10,od600fixmt)
plot(od600_wmn ~ c(1,4,3,2), pch =16, xlab="",ylab="",xaxt="n", yaxt="n", ylim=c(0.04,0.105),col=sourcecols ,xlim=c(0.5,4.5) ,cex=1.5 )
	arrows(c(1,4,3,2),od600_wmn-od600_wse, y1=od600_wmn+od600_wse, length=0,col=sourcecols,lwd=2)
	axis(side=1,at=c(1:4), sitenames,las=2)
	text(1:4,0.10,od600fixwt)
dev.off()
...
inocOD420mn <- tapply(treatfinal$OD420, treatfinal$isinoc,mean,na.rm=T)
inocOD420se <- tapply(treatfinal$OD420, treatfinal$isinoc,std.error)
inocOD600mn <- tapply(treatfinal$OD600, treatfinal$isinoc,mean,na.rm=T)
inocOD600se <- tapply(treatfinal$OD600, treatfinal$isinoc,std.error)
#plot figure
pdf("~/Inoculation.pdf",height=6,width=2)
layout(matrix(1:4,ncol=1))
par(mar=c(0,0,0,0))
par(oma=c(9,4,1,1))
plot(inocareamn~c(1:2),pch=21,bg=rgb(0.25,0.25,0.25), cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,2.75),ylim= c(10.25,12))#range(c(inocareamn-inocarease, inocareamn+inocarease)))
	arrows(c(1:2),inocareamn-inocarease,y1=inocareamn+inocarease,length=0,lwd=2)
	mtext(expression("Frond area, mm"^2), side=2,line=2)
	text(1.5,11.9,"p < 0.1")
plot(100*inocgreenmn~c(1:2),pch=21, bg=rgb(0.25,0.25,0.25), cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,2.75),ylim= 100*c(0.405,0.455)) #range(c(inocgreenmn-inocgreense, inocgreenmn+inocgreense)))
	arrows(c(1:2),100*(inocgreenmn-inocgreense),y1=100*(inocgreenmn+inocgreense),length=0,lwd=2)
	mtext("%Greenness", side=2,line=2)
	text(1.5,45.5,"*",cex=2)
plot(inocOD420mn~c(1:2),pch=21,bg=rgb(0.25,0.25,0.25),cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,2.75),ylim= c(0.05,0.13))#range(c(inocOD420mn-inocOD420se, inocOD420mn+inocOD420se)))
	arrows(c(1:2),inocOD420mn-inocOD420se,y1=inocOD420mn+inocOD420se,length=0,lwd=2)
	mtext("OD 420 nm", side=2,line=2)
	text(1.5,0.13,"*",cex=2)
	abline(h=mean(unique(treatfinal$OD420.BlankAverage)),lty=3) #even uninoculated microcosms > blanks; host cells may leave pigment in water
plot(inocOD600mn~c(1:2),pch=21,bg=rgb(0.25,0.25,0.25),cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,2.75),ylim= c(0.04,0.09))#range(c(inocOD600mn-inocOD600se, inocOD600mn+inocOD600se)))
...
LAOD420mnp <- tapply(inocOnly$OD420, inocOnly$NumSamePlant,mean,na.rm=T)
LAOD420sep <- tapply(inocOnly$OD420, inocOnly$NumSamePlant,std.error)
LAOD600mnp <- tapply(inocOnly$OD600, inocOnly$NumSamePlant,mean,na.rm=T)
LAOD600sep <- tapply(inocOnly$OD600, inocOnly$NumSamePlant,std.error)
...
LAOD420mn <- tapply(inocOnly$OD420, inocOnly$NumSameMicr,mean,na.rm=T)
LAOD420se <- tapply(inocOnly$OD420, inocOnly$NumSameMicr,std.error)
LAOD600mn <- tapply(inocOnly$OD600, inocOnly$NumSameMicr,mean,na.rm=T)
LAOD600se <- tapply(inocOnly$OD600, inocOnly$NumSameMicr,std.error)

#plot figure
pdf("~/LAtests_full.pdf",height=7,width=4.75)
layout(matrix(1:8,ncol=2, byrow=F))
par(mar=c(0,0,0,0))
par(oma=c(7,4,1,2))
plot(LAareamn~c(1:3),pch=16,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),
		ylim= c(10.25,13.5))
	arrows(c(1:3),LAareamn-LAarease,y1=LAareamn+LAarease,length=0,lwd=2)
	mtext(expression("Frond area, mm"^2), side=2,line=2)
	text(2,13.5,"p < 0.1")
plot(100*LAgreenmn~c(1:3),pch=16,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),
		ylim= 100*c(0.433,0.467))
	arrows(c(1:3),100*(LAgreenmn-LAgreense),y1=100*(LAgreenmn+LAgreense),length=0,lwd=2)
	mtext("%Greenness", side=2,line=2)
	text(2,46.7,"n.s.")
plot(LAOD420mnp~c(1:3),pch=16,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),
		ylim= c(0.08,0.15))
	arrows(c(1:3),LAOD420mnp-LAOD420sep,y1=LAOD420mnp+LAOD420sep,length=0,lwd=2)
	mtext("OD 420 nm", side=2,line=2)
	text(2,0.15,"p < 0.1")
plot(LAOD600mnp~c(1:3),pch=16,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),
		ylim= c(0.06,0.11))
	...
	mtext("Treatments match plants",side=1,line=5)
#
plot(LAareamnm~c(1:3),pch=16,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),yaxt="n",
		ylim= c(10.25,13.5))
	arrows(c(1:3),LAareamnm-LAareasem,y1=LAareamnm+LAareasem,length=0,lwd=2)
	text(2,13.5,"n.s.")
plot(100*LAgreenmnm~c(1:3),pch=16,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),yaxt="n",
		ylim= 100*c(0.433,0.467))
	arrows(c(1:3),100*(LAgreenmnm-LAgreensem),y1=100*(LAgreenmnm+LAgreensem),length=0,lwd=2)
	text(2,46.7,"n.s.")
plot(LAOD420mn~c(1:3),pch=16,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),yaxt="n",
		ylim= c(0.08,0.15))
	arrows(c(1:3),LAOD420mn-LAOD420se,y1=LAOD420mn+LAOD420se,length=0,lwd=2)
	text(2,0.15,"n.s.")
plot(LAOD600mn~c(1:3),pch=16,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),yaxt="n",
		ylim= c(0.06,0.11))
	arrows(c(1:3),LAOD600mn-LAOD600se,y1=LAOD600mn+LAOD600se,length=0,lwd=2)
	axis(side=1,at=c(1:3),labels=c("Neither", "One", "Both"),las=2)
	mtext("Treatments match microbes",side=1,line=5)
	text(2,0.11,"n.s.")
dev.off()
...
WLAod420mn <- tapply(inocOnly$OD420, paste(inocOnly$NumSameMicr,inocOnly$NumSameMicrWShed),mean,na.rm=T)
WLAod420se <- tapply(inocOnly$OD420, paste(inocOnly$NumSameMicr,inocOnly$NumSameMicrWShed),std.error)
WLAod600mn <- tapply(inocOnly$OD600, paste(inocOnly$NumSameMicr,inocOnly$NumSameMicrWShed),mean,na.rm=T)
WLAod600se <- tapply(inocOnly$OD600, paste(inocOnly$NumSameMicr,inocOnly$NumSameMicrWShed),std.error)
...
WLAod420mnp <- tapply(inocOnly$OD420, paste(inocOnly$NumSamePlant,inocOnly$NumSamePlantWShed),mean,na.rm=T)
WLAod420sep <- tapply(inocOnly$OD420, paste(inocOnly$NumSamePlant,inocOnly$NumSamePlantWShed),std.error)
WLAod600mnp <- tapply(inocOnly$OD600, paste(inocOnly$NumSamePlant,inocOnly$NumSamePlantWShed),mean,na.rm=T)
WLAod600sep <- tapply(inocOnly$OD600, paste(inocOnly$NumSamePlant,inocOnly$NumSamePlantWShed),std.error)

#plot
pdf("~/LA_watershed_tests.pdf",height=7,width=4.75)
WLAxaxt <- c(1,1.1,1.2,2,2.1,3)
WLAcolv <- c(1,0.5,0,0.5,0,0)
layout(matrix(1:8,ncol=2, byrow=F))
par(mar=c(0,0,0,0))
par(oma=c(7,4,1,2))
plot(WLAareamn~WLAxaxt,pch=21,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),
		ylim= c(10.25,13.5),bg=rgb(WLAcolv,WLAcolv,WLAcolv))
	arrows(WLAxaxt,WLAareamn-WLAarease,y1=WLAareamn+WLAarease,length=0,lwd=2)
	mtext(expression("Frond area, mm"^2), side=2,line=2)
	text(2,13.5,"local adaptation: *")
plot(100*WLAgreenmn~WLAxaxt,pch=21,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),
		ylim= 100*c(0.433,0.467),bg=rgb(WLAcolv,WLAcolv,WLAcolv))
	arrows(WLAxaxt,100*(WLAgreenmn-WLAgreense),y1=100*(WLAgreenmn+WLAgreense),length=0,lwd=2)
	mtext("%Greenness", side=2,line=2)
	text(2,46.7,"local adaptation: n.s.")
plot(WLAod420mnp~WLAxaxt,pch=21,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),
		ylim= c(0.08,0.15),bg=rgb(WLAcolv,WLAcolv,WLAcolv))
	arrows(WLAxaxt,WLAod420mnp-WLAod420sep,y1=WLAod420mnp+WLAod420sep,length=0,lwd=2)
	mtext("OD 420 nm", side=2,line=2)
	text(2,0.15,"local adaptation: n.s.")
plot(WLAod600mnp~WLAxaxt,pch=21,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),
		ylim= c(0.06,0.11),bg=rgb(WLAcolv,WLAcolv,WLAcolv))
	arrows(WLAxaxt,WLAod600mnp-WLAod600sep,y1=WLAod600mnp+WLAod600sep,length=0,lwd=2)
	mtext("OD 600 nm", side=2,line=2)
	text(2,0.11,"local adaptation: n.s.")
	axis(side=1,at=c(1:3),labels=c("Neither", "One", "Both"),las=2)
	mtext("Treatments match plants",side=1,line=5)
#
plot(WLAareamnm~WLAxaxt,pch=21,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),yaxt="n",
		ylim= c(10.25,13.5),bg=rgb(WLAcolv,WLAcolv,WLAcolv))
	arrows(WLAxaxt,WLAareamnm-WLAareasem,y1=WLAareamnm+WLAareasem,length=0,lwd=2)
	text(2,13.5,"local adaptation: n.s.")
plot(100*WLAgreenmnm~WLAxaxt,pch=21,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),yaxt="n",
		ylim= 100*c(0.433,0.467),bg=rgb(WLAcolv,WLAcolv,WLAcolv))
	arrows(WLAxaxt,100*(WLAgreenmnm-WLAgreensem),y1=100*(WLAgreenmnm+WLAgreensem),length=0,lwd=2)
	text(2,46.7,"local adaptation: n.s.")
plot(WLAod420mn~WLAxaxt,pch=21,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),yaxt="n",
		ylim= c(0.08,0.15),bg=rgb(WLAcolv,WLAcolv,WLAcolv))
	arrows(WLAxaxt,WLAod420mn-WLAod420se,y1=WLAod420mn+WLAod420se,length=0,lwd=2)
	text(2,0.15,"local adaptation: n.s.")
plot(WLAod600mn~WLAxaxt,pch=21,cex=2,ylab="",xlab="",xaxt="n",xlim=c(0.25,3.75),yaxt="n",
		ylim= c(0.06,0.11),bg=rgb(WLAcolv,WLAcolv,WLAcolv))
...
sqmm600W <- MCMCglmm(rhoOS~water-1,data=rhodat, verbose=F, nitt=100000,burnin=10000,thin=100)
	...
	siglabs[,3,2] <- c("*","*","","*")	
	#at 90% CI, rho higher in water M than water UM.
	HPDinterval(sqmm600W$Sol,0.9)
...
sqmmgreenW <- MCMCglmm(rhoGS~water-1,data=rhodat, verbose=F, nitt=100000,burnin=10000,thin=100)
 #nothing sig at 95% CI level, and all pos rho; but at 90%, rho in water WDM less positive than in water DR
	siglabs[,3,3] <- c("*","*","*","*")	
 	HPDinterval(sqmmgreenW$Sol,0.9)
...
plot(rhomns[,v,i]~c(1,4,3,2),ylim=c(-0.2,0.75),xlim=c(0.5, 4.5),pch=16,xaxt="n",yaxt="n",col=sourcecols)
	arrows(c(1,4,3,2),rhomns[,v,i]-rhoses[,v,i],y1=rhomns[,v,i]+rhoses[,v,i],length=0,col=sourcecols)
	abline(h=0,lty=3)
	text(c(1,4,3,2),0.72, siglabs[,v,i] )
	if(v==1){mtext(rholongnames[i], side=3,cex=0.7,line=0)}
	if(i==1){axis(side=2, at=c(0,0.5))}
